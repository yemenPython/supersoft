...
@section('content')
    <nav>
        <ol class="breadcrumb" style="font-size: 37px; margin-bottom: 0px !important;padding:0px">
            ...
            <li class="breadcrumb-item active"> {{__('Expense types')}}</li>
        </ol>
    </nav>
    ...
    <div class="col-md-12">
        ...
        <div class="col-md-12" style="margin-bottom: 20px">
            @include('admin.financial_management.tof_type_expenses.buttons')
        </div>
    ...
    <script type="application/javascript">
        ...
        function creatMainPartType(id) {
            $.ajax({
                dataType: 'json',
                type: 'GET',
                url: `{{ route("admin:financial_management.tof_type_expenses.create" ,['action_for' => 'create' ]) }}${id ? '?parent_id='+id : ''}`,
        ...
        function editPartType(id) {
            $.ajax({
                dataType: 'json',
                type: 'GET',
                url: `{{ route("admin:financial_management.tof_type_expenses.create" ,['action_for' => 'edit' ]) }}${id ? '&id='+id : ''}`,
                success: function (response) {
                    $("#form-modal").html(response.html_code)
                    $('#treeModal').modal('show');
                    $('#branchInTree').select2({
        ...
        function deletePartType(id) {
            swal({
                ...
                if (confirm_delete) {
                    window.location = "{{ url('admin/financial_management/tof_type_expenses/delete') }}"+"/"+id
                } else {
