...
@section('title')
    <title>{{ __('Super Car') }} - {{ __('Edit Assets Replacements') }} </title>
@endsection
...
@section('content')
    ...
    <nav>
            <ol class="breadcrumb" style="font-size: 37px; margin-bottom: 0px !important;padding:0px">
                ...
                <li class="breadcrumb-item active"> {{__('Edit Assets Replacements')}}</li>
            </ol>
        </nav>
        <div class="col-xs-12">
        <div class=" card box-content-wg-new bordered-all primary">
                <h4 class="box-title with-control" style="text-align: initial"><i class="fa fa-money"></i>  {{__('Edit Assets Replacements')}}
                ...
                <button class="control text-white" style="background:none;border:none;font-size:12px">{{__('Save')}}<img class="img-fluid" style="width:50px;height:50px;margin-top:-20px;margin-bottom:-13px" src="{{asset('assets/images/f1.png')}}"></button>
                    ...
                    <form  method="post" action="{{route('admin:assets_replacements.update', ['assetReplacement' => $assetReplacement->id])}}" class="form">
                        @csrf
                        @method('put')
