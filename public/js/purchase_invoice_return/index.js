function changeType() {
    ...
    if (invoice_type == 'from_supply_order') {
...
function calculateTotal() {
    ...
    for (let i = 1; i <= items_count; i++) {
        ...
        if ($('#price_' + i).length && $('#checkbox_item_' + i).is(':checked')) {
...
function selectSupplier(type) {
    ...
    if (type == 'from_supply_order') {

         discount = $('#p_r_supplier_discount').val();
         discount_type = $('#p_r_supplier_discount_type').val();

    }else {

         discount = $('#purchase_invoice_id').find(":selected").data('supplier-discount');
         discount_type = $('#purchase_invoice_id').find(":selected").data('supplier-discount-type');
    }
...
function supplierDiscount() {
    ...
    // let discount = $('#purchase_invoice_id').find(":selected").data('supplier-discount');
    // let discount_type = $('#purchase_invoice_id').find(":selected").data('supplier-discount-type');

    let discount = $('.supplier_discount').val();
    let discount_type = $('.supplier_discount_type_value').val();
...
function checkQuantity (index) {
    ...
    let max_quantity = $('#max_quantity_item_' + index).val();

    let item_quantity = $('#quantity_' + index).val();

    if (item_quantity > max_quantity) {

        swal({text: 'sorry, max quantity is : ' + max_quantity, icon: "error"});

        $('#quantity_' + index).val(max_quantity);
...
function selectPurchaseReceipt(index) {
