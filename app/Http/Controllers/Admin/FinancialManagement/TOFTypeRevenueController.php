...
use App\Models\FinancialManagement\TofTypeRevenue;
...
class TOFTypeRevenueController extends Controller
{
    /**
     * @var string
     */
    protected $viewPath = 'admin.financial_management.tof_type_revenues.';
    ...
    public function __construct(TreeService $treeService)
    {
        ...
        $this->model = new TofTypeRevenue();
        $this->route = route('admin:financial_management.tof_type_revenues.store');
    }
    ...
    public function create(Request $request)
    {
        ...
        try {
            if ($request->action_for == 'create') {
                ...
                $model = TofTypeRevenue::findOrFail($id);
                $formRoute = route('admin:financial_management.tof_type_revenues.update', $model->id);
    ...
    function update(TreeCreateRequest $request, TofTypeRevenue $tofTypeRevenue)
    {
        try {
            $this->treeService->editInDB($tofTypeRevenue, $request->all());
